...
int get_rand_int(int min, int max) { return (rand() % (max - min + 1) + min); }
...
s21_decimal bigint_to_decimal(__int128_t src) {
  ...
  for (int i = 0, k = 0; i < 3; i++)
    for (int j = 0; j < 32; j++, k++)
      if (IS_SET(src, k)) res.bits[i] = setBit(res.bits[i], j);
...
int setshortBit(short int n, unsigned int pos) { return (n | (1 << pos)); }

short int get_random_short(void) {
  short int val = 0;
  for (int i = 0; i < 16; i++)
    if (rand() % 2) val = setshortBit(val, i);
